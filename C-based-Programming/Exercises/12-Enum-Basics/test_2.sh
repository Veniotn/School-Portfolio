...
expected_output_1="That is 9.745 gallons."

# Use contains so we ignore additional output or input prompts
if grep -q "$expected_output_1" <<< "$output_1"; then
  eval "$passFormat"
  echo "Test #1 44.3(UK) - Pass: Output is correct"
else
  eval "$failFormat"
  echo "Test #1 44.3(UK) - Expected '$expected_output_1' but got: $output_1"
  returnStatus=1
fi
...
expected_output_2="That is 34.008 gallons."

# Use contains so we ignore additional output or input prompts
if grep -q "$expected_output_2" <<< "$output_2"; then
  eval "$passFormat"
  echo "Test #2 154.6(UK) - Pass: Output is correct"
else
  eval "$failFormat"
  echo "Test #1 154.6(UK) - Expected '$expected_output_2' but got: $output_2"
  returnStatus=1
fi
